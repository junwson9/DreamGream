  ...
  return (
    <div className="w-[360px] h-[800px] relative bg-white">
      <div ref={pageRef} className="w-[360px] h-[800px] relative bg-white">
        <div>
          <img
            src={Logo}
            alt=""
            className="absolute w-[200px] h-[200px] left-[75px]"
          />
        </div>
        <div className="w-48 h-48 left-[80px] top-[200px] absolute rounded-lg">
          <img
            src={imgURL}
            crossOrigin={process.env.REACT_APP_ORIGIN_URL}
            alt=""
            className="w-48 h-48 left-[0.01px] top-0 absolute bg-zinc-300 rounded-lg"
          />
          <div className="w-48 h-48 left-0 top-0 absolute" />
          <div className="w-48 mt-[200px] text-center text-zinc-800 text-base font-medium leading-snug">
            {sharedPost.title}
          </div>
        </div>
        {sharedPost && (
          <div className="left-[193px] top-[527px] absolute justify-center items-center gap-1.5 inline-flex">
            {sharedPost.profile_img ? (
              <img
                style={{ width: 30, height: 30, borderRadius: 9999 }}
                src={sharedPost.profile_img}
                alt=""
              />
            ) : (
        ...
        <div className="w-11 left-[65px] top-[531px] absolute text-zinc-500 text-base font-normal leading-snug whitespace-nowrap">
          작성자
        </div>
        <div className="w-11 left-[65px] top-[587.05px] absolute text-zinc-500 text-base font-normal leading-snug whitespace-nowrap">
          작성일
        </div>
        <div className="left-[193px] top-[587.05px] absolute text-zinc-800 text-base font-normal leading-snug whitespace-nowrap">
          {extractTimePart(sharedPost.created_date)}
        </div>
        <div className="left-[193px] top-[641px] absolute text-zinc-800 text-base font-normal leading-snug whitespace-nowrap">
          {sharedPost.dead_line}
        </div>
        <div className="left-[65px] top-[641px] absolute text-zinc-500 text-base font-normal leading-snug whitespace-nowrap">
          목표 시기
        </div>
      </div>
      <div className="w-64 h-8 px-8 py-1.5 left-[50px] top-[717px] absolute justify-center items-center gap-8 inline-flex">
        <button
          type="button"
          className="px-24 py-1.5 whitespace-nowrap text-indigo-400 text-xs font-bold leading-tight bg-white rounded-lg border border-indigo-400"
